...
const AddNewCategoryModal = () => {
      ...
      if (parentCategory) {
        if (newItemsSection) {
          ...
          if (parentCategoryIndex !== -1) {
            const subCategoryIndex = schemaStructure[
              parentCategoryIndex
            ]?.children?.findIndex(
            ...
            if (
              ...
              schemaStructure[parentCategoryIndex].children[subCategoryIndex]
                ?.children
            ) {
              schemaStructure[parentCategoryIndex].children[
                subCategoryIndex
          ...
          console.log(
            "schemaStructure[parentCategoryIndex]",
            schemaStructure[parentCategoryIndex],
          );
  ...
  const editSectionApiCall = async () => {
    try {
      ...
      const updateCategoryInTree = (
        ...
        for (const category of categories) {
          if (category.id === categoryId) {
            ...
            category.link = categorySlug;
            category.iconName = selectedIcon || "";
            return true;
          }

          if (category.children && category.children.length > 0) {
            const updated = updateCategoryInTree(category.children, categoryId);
      ...
      const updated = updateCategoryInTree(
        schemaStructure,
        addEditSectionModalData?.data?.id,
      );
      ...
      await fetchData({
        ...
        options: {
          onSuccess: async (data) => {
            setUserSchemaStructure(data);
            toast.success("Successfully updated the section!");
            handleOnCancel();

            const oldSlug = addEditSectionModalData?.data?.link;
            if (pathname?.includes(oldSlug) && oldSlug !== categorySlug) {
              router.push(pathname.replace(oldSlug, categorySlug));
            }
          },
        },
      });
  ...
  const [parentDropdownSections, setParentDropdownSections] = useState<
    Category[]
  >([]);
  ...
  }, [addEditSectionModalData, mainSections, userSchemaStructure]);
